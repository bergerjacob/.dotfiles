...
| `dotfiles-camera-usb-helper` | Root-owned low-level helper installed under `/usr/local/libexec` |
...
The PC profile provisions the narrowly scoped sudoers rule automatically when
you run `./setup.sh pc`. To reinstall it manually:

```bash
sudo setup-camera-sudo.sh
```

The PC profile links all camera commands into `~/.local/bin`.
...
bindsym $mod+Shift+c exec camera-mode toggle

# Launch Zoom for screenshare with Mod+Shift+Z
bindsym $mod+Shift+z exec zoom-screenshare
```
...
A: Run the one-time setup: `sudo setup-camera-sudo.sh`
